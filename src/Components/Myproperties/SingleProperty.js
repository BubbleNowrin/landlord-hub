...
const SingleProperty = () => {
  ...
  const uploadPhoto = (id, e) => {
    ...
    fetch(url, {
      ...
      const data = { img, id }
      console.log(data);
      fetch(`https://landlord-hub.vercel.app/update_image/${single?._id}`, {
        method: 'PUT',
        headers: {
          'content-type': 'application/json'
        },
        ...
        if (data.modifiedCount > 0) {
          fetch(`https://landlord-hub.vercel.app/property/${single._id}`)
            .then(res => res.json())
            .then(data => {
...

                    {
                      calc?.expense && <>
                        {calc?.receipt ? (
                          ...
                          <td>
                            {
                              loading ? <div className="w-8 h-8 border-4 border-dashed rounded-full animate-spin mx-auto"></div> : <input type="file" name="" id="" onChange={(e) => uploadPhoto(calc?._id, e)} />
                            }
                          </td>
                        )}
                      </>
